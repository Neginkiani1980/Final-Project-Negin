...
public class CraterApiCustomer {
    ...
    public void loginToCrater() {
        ...
        token = response.path("token"); //means "go into the API response and find the value of the key named "token"


    }

    @Test(dependsOnMethods = "loginToCrater")
    public void create_a_customer(){
        Faker faker = new Faker();

        String endPoint = "api/v1/customers";
        String name = faker.name().fullName(); //optional
